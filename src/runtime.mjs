...
import { buildToolPlan } from "./tools/tool-registry.mjs";
import { buildContextPlan } from "./context/context-plan.mjs";

export async function runTask(prompt,{dryRun=false,mode,maxCostUsd,preferJev=true,maxAttempts=2,allowWrites=false,registry=defaultModelRegistry,execute=executeById,verifier=new SwitchboardVerifier({preferJev})}={}){
  ...
  const toolPlan=buildToolPlan(routed.contract,{allowWrites});
  const contextPlan=buildContextPlan(routed.contract);
  await logEvent({type:"route",...routed,routingEconomics,toolPlan,contextPlan});
  if(dryRun)return{...routed,routingEconomics,toolPlan,contextPlan,attempts:[],execution:{dryRun:true}};
  ...
  for(let attemptNumber=1;attemptNumber<=Math.max(1,maxAttempts);attemptNumber+=1){
    ...
    const execution=await execute(executorId,prompt,{registry,toolPlan,contextPlan});
    ...
    let verification=null,verificationCostUsd=0;
    ...
    finalExecution=execution;finalVerification=verification;
  ...
  const result={...routed,routingEconomics,toolPlan,contextPlan,attempts,execution:finalExecution,verification:finalVerification,outcome:{accepted:success,attempts:attempts.length,totalKnownCostUsd,budgetUsd,budgetExceeded:totalKnownCostUsd>budgetUsd},totalElapsedMs:Math.round(performance.now()-started)};
