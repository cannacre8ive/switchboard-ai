export class JevRouter {
  constructor({apiKey=process.env.TYPESAFE_API_KEY,model=process.env.TYPESAFE_MODEL||"jev-latest"}={}){this.apiKey=apiKey;this.model=model;}
  ...
  async decide(prompt){
    ...
    const response=await client.systemOne({
      ...
      questions:{
        ...
        complexity:score("How difficult is this request to complete correctly?",["Trivial/routine","Straightforward","Moderately complex","Complex","Exceptionally difficult"]),
        risk:score("How costly would an incorrect result be?",["Low consequence","Minor consequence","Meaningful consequence","High consequence","Critical consequence"]),
    ...
    const probabilityOf=(answer,selected)=>{if(!answer)return 0.5;if(typeof answer.probability==="number")return answer.probability;if(answer.probabilities&&selected in answer.probabilities)return answer.probabilities[selected];return 0.5;};
    const yesProbability=(answer)=>{if(!answer)return 0.5;if(typeof answer.noul==="number")return answer.noul;if(typeof answer.probability==="number")return answer.probability;return 0.5;};
    const score01=(answer)=>{if(!answer)return 0.5;const raw=Number(answer.score);if(!Number.isFinite(raw))return 0.5;return raw<=1?Math.max(0,raw):Math.min(1,Math.max(0,raw/4));};
    const taskType=a.taskType?.choice??"other";
    const recommendedExecutor=a.executor?.choice??"cheap-general";
    const confidence=Math.min(probabilityOf(a.taskType,taskType),probabilityOf(a.executor,recommendedExecutor));
    const toolHints=[];
    ...
    return {
      source:"jev",taskType,complexity:score01(a.complexity),risk:score01(a.risk),confidence,
