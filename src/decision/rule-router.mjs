...
export class RuleRouter{
  async decide(prompt){
    const signals={automation:automation.test(prompt),codingStrong:codingStrong.test(prompt),codingWeak:codingWeak.test(prompt),research:research.test(prompt),writing:writing.test(prompt),data:data.test(prompt),planning:planning.test(prompt),retrieval:retrieval.test(prompt),extraction:extraction.test(prompt)};
    let taskType="other";
    ...
    return {
      source:"rules",taskType,complexity,risk:risky?0.8:0.15,confidence:recognized?0.76:0.55,
      needsWeb,needsCode,needsLongContext,
